...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    case 'GET':
      return getMachineById();
    case 'PUT':
      return updateMachineById();
    case 'DELETE':
      return deleteMachineById();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getMachineById() {
    const data = await prisma.machine.findFirst(convertQueryToPrismaUtil(req.query, 'machine'));
  ...
  async function deleteMachineById() {
